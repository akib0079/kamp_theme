...
class ProductCardAltV2 extends Component {
  #slider = null;
  connectedCallback() {
  ...
  initSlider() {
    ...
    this.addEventListener('variant_changed', (e) => {
      ...
      const mediaId = this.querySelector(`input[value="${variantId}"]`).dataset.media;
      if (!mediaId) return;
      const slideIndex = [...this.#slider.slides].findIndex(
  ...
  getSelectedOptions() {
    const selectors = this.querySelectorAll('.product-alt__option-selector');
    const values = [];
...
if (!customElements.get('product-card-alt-v2')) {
  customElements.define('product-card-alt-v2', ProductCardAltV2);
}